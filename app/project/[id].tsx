...
export default function ProjectBoardScreen() {
  ...
  const [originalListId, setOriginalListId] = useState<number | null>(null); // Para detectar cambio de lista
  ...
  const openEditTaskModal = (task: Tarea) => {
    ...
    setOriginalListId(task.lista_id);
    setTaskForm({
      titulo: task.titulo,
  ...
  const updateTask = async () => {
    ...
    try {
      ...
      // Si cambió de lista, usar el endpoint move
      if (originalListId !== selectedListId) {
        // Paso 1: Mover la tarea a la nueva lista
        const listaDestino = listas.find((l) => l.id === selectedListId);
        const moveData = await tasksAPI.move(editingTask!.id, {
          nueva_lista_id: selectedListId!,
          nuevo_orden: listaDestino?.tareas.length || 0, // Al final de la lista
        });

        if (!moveData.success) {
          Alert.alert('Error', moveData.message || 'No se pudo mover la tarea');
          setSavingTask(false);
          return;
        }

        // Paso 2: Actualizar los demás datos de la tarea
        const updateData = await tasksAPI.update(editingTask!.id, taskForm);

        if (!updateData.success) {
          Alert.alert('Error', updateData.message || 'No se pudo actualizar la tarea');
          setSavingTask(false);
          return;
        }

        updatedTask = updateData.data;

        // Actualizar estado local: remover de lista original y agregar a nueva
        setListas(
          listas.map((lista) => {
            // Remover de la lista original
            if (lista.id === originalListId) {
              return {
                ...lista,
            ...
            if (lista.id === selectedListId) {
              return {
                ...lista,
                tareas: [...lista.tareas, updatedTask],
              };
            }
            return lista;
          })
        ...
        // Si NO cambió de lista, solo actualizar los datos
        ...
        // Actualizar estado local: solo actualizar la tarea en su lista
        setListas(
          listas.map((lista) => ({
            ...lista,
            tareas: lista.tareas.map((t) => (t.id === editingTask!.id ? updatedTask : t)),
          }))
        );
      }
  ...
  return (
    <GestureHandlerRootView style={{ flex: 1 }}>
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        {/* Header */}
        <View style={[styles.header, { backgroundColor: colors.card, borderBottomColor: colors.cardBorder,paddingTop:40 }]}>
            ...
            >
              <View style={[styles.modalContent, styles.modalContentLarge, { backgroundColor: colors.card }]}>
                ...
                <View style={styles.modalBody}>
                  ...
                  {/* Selector de Lista - SOLO al editar */}
                  {editingTask && (
                    <View style={styles.inputGroup}>
                      <Text style={[styles.label, { color: colors.text }]}>
                        Lista {originalListId !== selectedListId && '(modificada)'}
                      </Text>
                      <View style={styles.listSelectorContainer}>
                        {listas.map((lista) => (
                          <TouchableOpacity
                            key={lista.id}
                            style={[
                              styles.listSelectorChip,
                              {
                                backgroundColor:
                                  selectedListId === lista.id ? colors.accent : colors.input,
                                borderColor: colors.inputBorder,
                              },
                            ]}
                            onPress={() => setSelectedListId(lista.id)}
                          >
                            <Ionicons
                              name={selectedListId === lista.id ? 'checkmark-circle' : 'list-outline'}
                              size={16}
                              color={selectedListId === lista.id ? '#FFFFFF' : colors.text}
                            />
                            <Text
                              style={[
                            ...
                            {originalListId === lista.id && selectedListId !== lista.id && (
                              <View style={[styles.originalBadge, { backgroundColor: colors.textSecondary }]}>
                                <Text style={styles.originalBadgeText}>Actual</Text>
                              </View>
                            )}
                          </TouchableOpacity>
                        ))}
